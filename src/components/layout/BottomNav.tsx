...
export const BottomNav: React.FC = () => {
  ...
  const mainTabs: { tab: NavTab; label: string; icon: React.ReactNode }[] = [
    ...
    { tab: 'students', label: '원생', icon: <Users className="w-5 h-5" /> },
    { tab: 'timetable', label: '시간표', icon: <Clock className="w-5 h-5" /> },
  ...
  const moreTabs: { tab: NavTab; label: string; icon: React.ReactNode; requiresDirector?: boolean }[] = [
    ...
    { tab: 'parents', label: '학부모 관리', icon: <UserSquare2 className="w-5 h-5" /> },
    { tab: 'classes', label: '반/수업 관리', icon: <GraduationCap className="w-5 h-5" /> },
    ...
    { tab: 'teachers', label: '선생님 관리', icon: <GraduationCap className="w-5 h-5" />, requiresDirector: true },
