...
export const StudentDetailInfoTab: React.FC<StudentDetailInfoTabProps> = ({
  ...
  const levelLabel = isAcademy ? '과목 / 과정' : '피아노 과정/레벨';
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200/80 space-y-3">
        ...
        <div className="space-y-2 text-xs">
          ...
          {student.leaveDate && (
            <div className="flex justify-between py-1.5 border-t border-slate-200/60">
              <span className="text-slate-500">휴원/퇴원일</span>
              <span className="text-slate-800 font-medium">{student.leaveDate}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200/80 space-y-3">
        ...
        <div className="space-y-2 text-xs">
          ...
          <div
            className={`flex justify-between py-1.5${!isAcademy ? ' border-b border-slate-200/60' : ''}`}
          >
