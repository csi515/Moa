...
import type { SaleStockDeductLine } from '../types/inventory';
...
/**
 * 판매 반품.
 * 원본 Sale 삭제·금액 덮어쓰기 없음. 포인트·PG 미연동.
 */
export const saleReturnService = {
  /** sale_item_id → 누적 반품 수량 */
  ...
  /**
   * 반품 처리: sale_returns 기록 → 재고 복구(StockMovement return).
   * 원본 sales / sale_items 불변.
   */
  async createReturn(input: SaleReturnCreateInput): Promise<SaleReturnWithItems> {
    ...
    if (stockLines.length > 0) {
      await inventorySaleStock.applyReturns(orgId, saleReturnId, stockLines);
    }
