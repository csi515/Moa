...
export const GymDashboardView: FC = () => {
  ...
  const isVisible = useMemo(
    () => (id: string) => isDashboardWidgetVisible(id, settings, 'gym'),
    [settings]
  );
  ...
  const showRecentMembers = isVisible('panel_recent_members');
  const showAttendanceSummary = isVisible('panel_attendance_summary');
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {(showRecentMembers || showAttendanceSummary) && (
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="text-xs font-bold text-orange-600 hover:underline min-h-[44px] px-2"
                >
                  전체 보기
                </button>
              </div>
              {students.length === 0 ? (
                <EmptyState
                  ...
                  description="회원을 등록하고 수업 레벨, 보호자, PIN을 설정하세요."
                  action={
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  {[...students]
                    ...
                    .map((s) => (
                      <button
                        ...
                        className="w-full text-left p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-orange-200 hover:bg-orange-50/30 transition-colors min-h-[44px]"
                      >
                        <p className="font-bold text-slate-900 text-sm">{s.name}</p>
                        <p className="text-xs text-slate-500 mt-0.5">
                          {s.level} · 등록 {s.joinDate.slice(0, 10)}
                        </p>
                      </button>
                    ))}
                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                  <p className="text-xs text-slate-500 font-semibold">재적 회원</p>
              ...
              <button
                type="button"
                onClick={() => setActiveTab('attendance')}
                className="mt-4 w-full py-3 min-h-[44px] rounded-xl border border-orange-200 text-orange-700 text-xs font-bold hover:bg-orange-50 transition-colors"
              >
                출입 관리 열기
              </button>
            </div>
