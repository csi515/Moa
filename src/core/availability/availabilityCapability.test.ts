...
function run() {
  ...
  const capabilitySrc = readSrc('src/core/availability/availabilityCapability.ts');
  ...
  const skinFacade = readSrc('src/modules/skin/staffHours.ts');
  assert.match(skinFacade, /@\/core\/availability\/windows/);
  assert.doesNotMatch(skinFacade, /function isOutsideStaffHours/);

  const pilatesValidate = readSrc('src/modules/pilates/components/bookings/validateBookingCreate.ts');
