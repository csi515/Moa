-- approve_guardian_enrollment: 기존 enrollment(customer) 재사용
-- - active/leave: 기존과 같이 거부
-- - withdrawn/alumni 등 이력이 있고 customer가 살아 있으면 재활성화 (새 Customer INSERT 방지)
-- - 학원 선등록 orphan Customer와의 이름 기반 자동 병합은 하지 않음 (Guardian Link 경로 사용)
-- - metadata에 globalStudentId 기록
...
BEGIN
  ...
  -- 동일 student+org enrollment가 있으면 Customer 재사용 또는 교체 후 재활성화
  -- (UNIQUE(student_id, organization_id) 때문에 INSERT 대신 UPDATE)
  SELECT se.*
