-- create_sale에 포인트 redeem을 동일 트랜잭션으로 포함
-- sales.points_used 와 point_transactions(redeem) 불일치 방지
...
BEGIN
  ...
  -- points_used > 0: 동일 트랜잭션에서 ledger redeem (실패 시 판매·재고 전체 rollback)
  IF v_points > 0 THEN
    SELECT COALESCE((o.settings->'features'->'points'->>'enabled') = 'true', false)
      INTO v_points_enabled
    FROM core.organizations o
    WHERE o.id = p_organization_id;

    IF NOT COALESCE(v_points_enabled, false) THEN
      RAISE EXCEPTION '포인트 사용이 비활성화되어 있습니다.';
    END IF;
    ...
    IF COALESCE((v_redeem->>'skipped')::boolean, false) THEN
      RAISE EXCEPTION '포인트 사용 처리에 실패했습니다 (%).',
        COALESCE(v_redeem->>'reason', 'unknown');
    END IF;
  END IF;
...
COMMENT ON FUNCTION core.create_sale(UUID, UUID, core.payment_method, NUMERIC, JSONB) IS
  '판매+재고+포인트사용(redeem) 원자 RPC. points_used>0이면 apply_point_redeem_for_sale 포함, 실패 시 전체 rollback.';
